...
Ключу нужны права на чтение/запись members и front. Если у ключа пока нет прав на запись front, можно временно поставить `FLORALITY_SYNC_FRONT_ENABLED=false`: бот не будет отправлять фронт во Florality. Импорт личностей запускается вручную через **Управление личностями → Импорт из Florality**; перед импортом SQLite-база копируется в `data/backups` с датой в имени файла. Если личность удалили во Florality, импорт перенесёт её в локальную категорию `Deleted`. Если фронт меняют во Florality, бот проверяет `GET /front` раз в `FLORALITY_PULL_INTERVAL_SECONDS` секунд; если фронтер ещё не найден локально, бот импортирует эту личность из ответа Florality и обновляет локальный фронт. Новые личности во Florality автоматически не создаются, пока `FLORALITY_CREATE_MISSING_MEMBERS_ENABLED=false`. Бот хранит локальное соответствие `member_id` к Florality ID в отдельной служебной таблице и не меняет структуру Simply Plural-данных.
...
Основные таблицы Simply Plural не требуют миграций для обычной работы. При работе через Florality локальная база обновляется импортом, а удалённые во Florality личности переносятся в категорию `Deleted` без физического удаления строки из базы.
