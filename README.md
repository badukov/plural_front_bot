...
История фронта хранится отдельно в SQLite как сжатые gzip+base64 JSON-снимки текущего фронта после каждого изменения. Снимки собственной истории бота старше `FRONT_HISTORY_ACTIVE_DAYS` переносятся из активной таблицы в архивную, но не удаляются; архив участвует в расширенной статистике. Статистика вычисляет не только число переключений, но и длительность каждого непрерывного пребывания личности на фронте между соседними снимками. Дополнительно бот при первом запуске постепенно загружает историю сессий Florality, затем раз в `FLORALITY_HISTORY_PULL_INTERVAL_SECONDS` перепроверяет последние `FLORALITY_HISTORY_ACTIVE_DAYS` дней и обновляет сессии по их стабильному Florality ID — поэтому исправленное задним числом время не создаёт дубль. Завершённые сессии Florality старше активного окна также переносятся в свою архивную таблицу SQLite. Раздел истории показывает имя, начало и конец сессии, затем длительность; начало и конец передаются в нативном формате времени Telegram, поэтому каждый пользователь видит их в своём часовом поясе. Кнопка статистики показывает последние 30 дней; команды `/stats 90` и `/stats all` читают в том числе архивы. В «Инфо о фронте» бот отправляет доступные аватары текущих фронтеров, а при уведомлении об изменении — аватар затронутой личности; при блюре показываются аватары снятых фронтеров.
